"""Interactive semantic matcher (CLI).

Loads the same tutor data the live app serves (tutors.json), embeds every
tutor's expertise, then for each typed request: embeds it, hard-filters by
availability, and re-ranks the survivors by a weighted blend of meaning,
rating, and price.
"""
import json
...
# --- Single source of truth: same file the engine and benchmark use ---
with open("tutors.json", "r", encoding="utf-8") as f:
    tutors = json.load(f)

tutor_embeddings = model.encode([t["expertise"] for t in tutors])

# Student preferences (stand-in for what the UI would collect).
STUDENT_TZ = -8
STUDENT_TIME = (18, 21)
